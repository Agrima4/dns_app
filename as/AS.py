...
def main():
    ...
    log.info(f"UDP server is up and listening on address"
             f"{socket.gethostbyname(socket.gethostname())}:{SERVER_PORT}")

    while (True):
        msg_bytes, client_addr = udp_socket.recvfrom(BUFFER_SIZE)
        mssg = pickle.loads(msg_bytes)
        log.info(f"Message from Client: {mssg!r}")
        if len(msg) == 4:
            ...
            log.error(mssg)
            udp_socket.sendto(mssg, client_addr)
